...
namespace FCAProj.FCACLASS
{
    public class CustomerRegistrationClass
    {
       ...
       public DataTable fetchloandata()
        {
            OpenConection();


            DataTable dispitem = new DataTable(); 
            SqlCommand command = new SqlCommand("select Creditamount,Date from tbl_goldinterest where GoldNo=@gno order by Creditamount Asc   ", con);
            command.Parameters.AddWithValue("@gno", goldno);
        ...
        private int mid;
        private double rate;
        public DataTable fetchrate()
        {
            OpenConection();


            DataTable dispitem = new DataTable();
            SqlCommand command = new SqlCommand("select L_interestrate from tblLoantype where B_id=@mid ", con);
        ...
        public void InsertgoldInterest()
        {
            OpenConection();

             paydate= System.DateTime.Now.ToString("dd/MM/yyyy");
            DateTime date = Convert.ToDateTime(paydate);
            string qry1 = "insert into tbl_goldinterest values (@creditamount,@amounttopay,@date,@payamount,@gno);";
            SqlCommand cmd1 = new SqlCommand(qry1, con);
            cmd1.Parameters.AddWithValue("@creditamount", creditamount);
            cmd1.Parameters.AddWithValue("@amounttopay", goldloanamount);
            cmd1.Parameters.AddWithValue("@date", paydate);
            cmd1.Parameters.AddWithValue("@payamount", paidamount);
            cmd1.Parameters.AddWithValue("@gno", goldno);
            cmd1.ExecuteNonQuery();


        }

        public DataTable Viewinterstdetails()
        {
            OpenConection();


            DataTable interest = new DataTable();
            SqlCommand command = new SqlCommand("select Creditamount,PaidAmount,Date,Amount_to_pay from tbl_goldinterest where GoldNo=@gno  ", con);
            command.Parameters.AddWithValue("@gno", goldno);
        ...
        public int CheckMail()
        {
            ...
            SqlCommand command = new SqlCommand("select count(C_email) from tblCustomerregistration where C_email=@email ", con);
            int count= 0;
            command.Parameters.AddWithValue("@email", cemail);
            object cnt = command.ExecuteScalar();
            if (cnt != DBNull.Value)
            {
        ...
        public int CheckUsername()
        {
            ...
            SqlCommand command = new SqlCommand("select count(Username) from tblLogin where Username=@uname ", con);
            int count = 0;
            command.Parameters.AddWithValue("@uname", cusername);
            object cnt = command.ExecuteScalar();
            if (cnt != DBNull.Value)
            {
                count = Convert.ToInt32(cnt);
        ...
        public DataTable readcusname()
        {
            ...
            SqlCommand command = new SqlCommand("select C_name from tblCustomerregistration where C_id=@id ", con);
