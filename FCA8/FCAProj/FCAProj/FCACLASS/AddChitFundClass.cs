    ...
    public class AddChitFundClass
    {
       ...
       // public string Sdate { get => sdate; set => sdate = value; }
        ...
        public DataTable CustomerChitsView()
        {
            ...
            DataTable dtReg = new DataTable();
            SqlCommand command = new SqlCommand("Select ch.Chit_no ,ch.Chit_amount,ch.Chit_installment, ch.chit_Stardate,aut.Status,aut.Installment_no from tblChitfund ch, tbl_Chitauction aut,  tbl_chitmembers c where ch.Chitfund_id = aut.Chitfund_id and ch.Chitfund_id = c.Chit_No and c.C_id=@cid and  CAST(getdate() as date) <= CAST(aut.Auction_close_time as date)", con);
        ...
        public DataTable CustomerChitsView1()
        {
            ...
            DataTable dtReg = new DataTable();
            SqlCommand command = new SqlCommand("Select ch.Chit_no ,ch.Chit_amount,ch.Chit_installment, ch.chit_Stardate from tblChitfund ch, tbl_chitmembers c where ch.Chitfund_id = c.Chit_No and c.C_id=3 ", con);
            command.Parameters.AddWithValue("@cid", cusid);
            //command.Parameters.AddWithValue("@accountName", accountName);
            SqlDataAdapter da = new SqlDataAdapter(command);// this will query your database and return the result to your datatable
