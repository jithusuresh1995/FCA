...
namespace FCAProj.FCACLASS
{
    public class AddChitFundClass
    {
        ...
        public DataTable CustomerChitsView()
        {
            ...
            DataTable dtReg = new DataTable();
            SqlCommand command = new SqlCommand("Select ch.Chit_no ,ch.Chit_amount,ch.Chit_installment, ch.chit_Stardate,aut.Status,aut.Installment_no,aut.Auct_Owned from tblChitfund ch, tbl_Chitauction aut,  tbl_chitmembers c where ch.Chitfund_id = aut.Chitfund_id and ch.Chitfund_id = c.Chit_No and c.C_id=@cid and  CAST(getdate() as date) <= CAST(aut.Auction_close_time as date)", con);
